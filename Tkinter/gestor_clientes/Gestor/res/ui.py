...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        Label(frame, text="Nombre (2 ints y 30 chars)").grid(row=0, column=1)
        Label(frame, text="Apellido (2 ints y 30 chars)").grid(row=0, column=2)
        ...
        crear = Button(frame, text="Crear", command=self.create_client, width=8)
    ...
    def create_client(self):
        ...
        self.close() #Con esta funcion se cierra el programa despues de crear el nuevo cliente.
    ...
    def validate(self, event, index):
        ...
        #     else:
        #         event.widget.configure({"bg":"Red"})
        valido = res.helpers.dni_valido(valor, db.Clientes.lista) if index == 0 \
